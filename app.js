// Apps Script 웹 앱 URL로 교체하세요 (예: https://script.google.com/macros/s/AKfycbx.../exec)
...
function pageSize(){ return window.matchMedia('(max-width: 768px)').matches ? 24 : 32; }
function onResizeReflow(){
  const newLimit = pageSize();
  if (state.limit !== newLimit){
    state.limit = newLimit; state.offset = 0; fetchAndRender(true);
  }
}
...
function renderItems(items){
  ...
  items.forEach(it => {
    ...
    btn.className = 'emoji';
    btn.textContent = it.emoji; // 이름/설명은 화면에 숨김
    btn.title = it.emoji;
...
function toggleMore(show){ document.getElementById('more').hidden = !show; }
